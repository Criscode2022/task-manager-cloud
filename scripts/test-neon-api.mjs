...
async function main() {
  ...
  const verify = await req('POST', `/users/${user.id}/verify`, {
    pin_hash: pinHash,
  });
  if (!verify.valid) throw new Error('PIN verify failed');
  console.log('✓ verify pin');

  const byPin = await req('GET', `/users/by-pin/${encodeURIComponent(pinHash)}`);
  ...
  const updated = await req('PUT', `/tasks/${task.id}`, {
    done: true,
    priority: 'medium',
  });
  ...
  const listed = await req('GET', `/tasks?userId=${user.id}`);
  ...
  const bulk = await req('POST', '/tasks/bulk', {
    tasks: [
      {
        user_id: user.id,
        title: 'Bulk A',
  ...
  await req('DELETE', `/tasks?userId=${user.id}`);
